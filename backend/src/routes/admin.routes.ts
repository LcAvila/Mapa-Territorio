...
router.post('/users', async (req, res) => {
  const { username, password, role, tipo, full_name, repCode } = req.body;
  ...
  const user = await prisma.user.create({ data: { username, password: hashedPassword, role: role || 'user', tipo: tipo || 'representante', full_name, repCode }, select: PUBLIC_USER_FIELDS });
  res.status(201).json(user);
});

router.put('/users/:id', async (req, res) => {
  const id = Number(req.params.id);
  const { username, password, role, repCode, tipo, full_name } = req.body;
